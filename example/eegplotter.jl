...
"""
    signalchannels(edfh; max_channels=8)
...
non-annotation channels are kept, since `filteredpages` builds a single
...
"""
function signalchannels(edfh; max_channels = 8)
    candidates = Int[]
...
"""
    filteredpages(edfh; channels, seconds_per_page)
...
"""
function filteredpages(edfh; channels, seconds_per_page = 15.0)
...
"""
    averagereference(edfh, channels)
...
overall" summary.
"""
function averagereference(edfh, channels)
...
function vieweeg(filename; seconds_per_page = 15.0, max_channels = 8)
    ...
    # Time window of the currently displayed page, as fraction of spectrogram's time span
    page_t0 = @lift spec_t0 + (($page_no - 1) / n_pages) * (spec_t1 - spec_t0)
    page_t1 = @lift spec_t0 + (min($page_no / n_pages, 1.0)) * (spec_t1 - spec_t0)
    ...
    on(events(fig).mousebutton) do event
        if event.button == Mouse.left && event.action == Mouse.press && is_mouseinside(spec_ax)
            ...
            frac = clamp((t - t0) / (t1 - t0), 0.0, 1.0)
            newp = clamp(floor(Int, frac * n_pages) + 1, 1, n_pages)
            if newp != page_no[]
                page_no[] = newp
            end
        end
